...
class Index extends Base
{
    ...
    public function index()
    {
        ...
        if(isset($param['personCount'])){
            Session::set('personCount',$param['personCount']);
        }
        /*
        if(!Session::has('personCount')){
            $this->assign('param',$param);
            return $this->fetch('personCount');
        }*/
        ...
        //展示暂时使用默认店铺
        $param['contactNo'] = !empty($param['contactNo'])?$param['contactNo']:'099';
        $param['contactMemberNo'] = !empty($param['contactMemberNo'])?$param['contactMemberNo']:'33_1';
        if ($login) {
    ...
    //根据菜品id 获取单个菜品规格
    public function getGoodSpecByid(){
        $contactNo = Session::get('contact');
        $contactNo = empty($contactNo)?input('param.contact'):$contactNo;
        $goodsid = input('param.goodsid');
        $result = get_food_list($contactNo,1,$goodsid);
        $food_secs = $result['foodlist'];
        return json($food_secs);
    }
    ...
    public function payOrder()
    {
        ...
        if ($request->isPost()) {
            ...
            if (isset($return['code']) && $return['code'] == 1) {
                ...
                Session::delete('order');
                $this->success('支付成功!', url('index/allorder'));
            ...
            Session::set('show_order', $show_order);
            /*获取用户类型配置，微信支付宝或者其它*/
            $user_type = config('user_type');
            $user = Session::get('web_user');
            $contact = Session::get('contact');
            $personCount = Session::get('personCount');
            $contact_info = Db::name('contact')
                ->field('*')
                ->where('number', $contact)
                ->find();
            if($contact_info['is_cover_charge']){
                $contact_info['personCount'] = $personCount;
            }
            //微信，支付宝用户才能看到这两种支付方式
    ...
    {
        $key = input('param.orderSN');
        $key = (int)$key-1;
        $show_order = Session::get('show_order');
        $orderIndex = $show_order[$key]['orderIndex'];
        unset($show_order[$key]);
        $show_order_new=[];
        //套餐菜品提取重组
        foreach($show_order as $v)
        {
            $meal = [];
            if(isset($v['_food'])&&!empty($v['_food'])){
                $meal = $v;
                unset($meal['_food']);
                $show_order_new[] = $meal;
                foreach($v['_food'] as $food)
                {
                    $show_order_new[] = $food;
                }
            }else{
        ...
        //重新计算总价
        $totalPrice = 0;
        foreach ($show_order_new as $f) {
            $good_total = $f['counter'] * $f['salePrice'];
            $totalPrice += $good_total;
        }
        $order = Session::get('order');
        $order_order = json_decode($order['order'],true);
        foreach($order_order as $o)
        {
            if($o['orderIndex'] == $orderIndex){
                unset($order_order[$orderIndex]);
            }
        }
        $order['order'] = json_encode($order_order);
        $order['carts'] = $show_order_new;
...
//        $order['order'] = [];
    ...
    public function getOrder()
    {
        ...
        $size = 5;
        $user = Session::get('web_user');
        $orderlist = get_user_order($user['user_type'],$user['openid'],$page,$size);
        if($orderlist['code']){
    ...
    public function takemeal()
    {
        ...
        return $this->fetch('takemeal');
    }
    public function paychoose()
    {

        return $this->fetch('paychoose');
    }
    public function payreminder()
    {

        return $this->fetch('payreminder');
    }
    public function paycompleted()
    {

        return $this->fetch('paycompleted');
    }
