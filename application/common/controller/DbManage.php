...
class DbManage
{
     ...
     */
    public static function backup($table, $dir = "backupTable")
    {
        $dir = RUNTIME_PATH . $dir . DS . date('Ymd') . DS;
        if (!file_exists($dir)) {
            mkdir($dir, 0755, true);
        }
        $filename = $table . ".sql";
    ...
    private static function _retrieve()
    {
        ...
        $value .= '-- Created by DbManage class, Power By zhonchengcheng. ' . self::$ds;
    ...
    private static function _insertTableStructure($table)
    {
        ...
        $res = self::get("SHOW CREATE TABLE `{$table}`;");
    ...
    private static function _insertRecord($table, $pageSize)
    {
        ...
        for ($i = 0; $i < $pageSize; $i++) {
            $rows       = Db::name($table)->limit($i, self::$size)->select();
            $delimiter1 = "";
            foreach ($rows as $row) {
                $sql .= "{$delimiter1}(";
                $delimiter2 = "";
